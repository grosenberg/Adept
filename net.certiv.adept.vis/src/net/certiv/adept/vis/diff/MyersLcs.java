...
public abstract class MyersLcs {

	// 10^8, the value of N*M when to start binding the run time
	...
	/**
	 * Myers' algorithm for longest common subsequence. O((M + N)D) worst case time, O(M + N + D^2)
	 * expected time, O(M + N) space (http://citeseer.ist.psu.edu/myers86ond.html) Note: Beyond
	 * implementing the algorithm as described in the paper I have added diagonal range compression
	 * which helps when finding the LCS of a very long and a very short sequence, also bound the running
	 * time to (N + M)^1.5 when both sequences are very long. After this method is called, the longest
	 * common subsequence is available by calling getResult() where result[0] is composed of entries
	 * from l1 and result[1] is composed of entries from l2
	 */
	public void longestCommonSubsequence() {
		...
		/*
		 * The common prefixes and suffixes are always part of some LCS, include them now to reduce our
		 * search space
		 */
		int forwardBound;
		...
		this.length = forwardBound + length1 - backBoundL1 - 1 + lcs_rec(forwardBound, backBoundL1, forwardBound,
				backBoundL2, new int[2][length1 + length2 + 1], new int[3]);
	...
	/**
	 * The recursive helper function for Myers' LCS. Computes the LCS of l1[bottoml1 .. topl1] and
	 * l2[bottoml2 .. topl2] fills in the appropriate location in lcs and returns the length
	 *
	 ...
	 * @param V should be allocated as int[2][l1.length + l2.length + 1], used to store furthest
	 *            reaching D-paths
	 * @param snake should be allocated as int[3], used to store the beginning x, y coordinates and the
	 *            length of the latest snake traversed
	 * @param lcs should be allocated as TextLine[2][l1.length], used to store the common points found
	 *            to be part of the LCS where lcs[0] references lines of l1 and lcs[1] references lines
	 *            of l2.
	 * @return the length of the LCS
	 */
	private int lcs_rec(int bottoml1, int topl1, int bottoml2, int topl2, int[][] V, int[] snake) {
		...
		if (d > 1) {
			return len + lcs_rec(bottoml1, startx - 1, bottoml2, starty - 1, V, snake)
					+ lcs_rec(startx + len, topl1, starty + len, topl2, V, snake);
		} else if (d == 1) {
			/*
			 * In this case the sequences differ by exactly 1 line. We have already saved all the lines after
			 * the difference in the for loop above, now we need to save all the lines before the difference.
			 */
	...
	/**
	 * Helper function for Myers' LCS algorithm to find the middle snake for l1[bottoml1..topl1] and
	 * l2[bottoml2..topl2] The x, y coodrdinates of the start of the middle snake are saved in snake[0],
	 * snake[1] respectively and the length of the snake is saved in s[2].
	 *
	 * @param l1 The 1st sequence
	 * @param bottoml1 Index in the 1st sequence to start from (inclusive)
	 * @param topl1 Index in the 1st sequence to end on (inclusive)
	...
	private int find_middle_snake(int bottoml1, int topl1, int bottoml2, int topl2, int[][] V, int[] snake) {
		...
		for (int d = 0; d <= limit; d++) {
			...
			for (int k = start_diag; k <= end_diag; k += 2) {
				...
				// d: " + d );
				while (x < N && y < M && isRangeEqual(x + bottoml1, y + bottoml2)) {
					x++;
					y++;
					snake[2]++;
				}
			...
			for (int k = start_diag; k <= end_diag; k += 2) {
				...
				// d: " + d);
				while (x > 0 && y > 0 && isRangeEqual(x - 1 + bottoml1, y - 1 + bottoml2)) {
					x--;
					y--;
					snake[2]++;
				}
		...
		/*
		 * computing the true LCS is too expensive, instead find the diagonal with the most progress and
		 * pretend a midle snake of length 0 occurs there.
		 */
		...
		snake[2] = 0;
		return 5; /*
					 * HACK: since we didn't really finish the LCS computation we don't really know the length of the
					 * SES. We don't do anything with the result anyway, unless it's <=1. We know for a fact SES > 1 so
					 * 5 is as good a number as any to return here
					 */

	}

	/**
	 * Takes the array with furthest reaching D-paths from an LCS computation and returns the x,y
	 * coordinates and progress made in the middle diagonal among those with maximum progress, both from
	 ...
	 * @param limit the number of steps made in an attempt to find the LCS from the front and back
	 * @param V the array storing the furthest reaching D-paths for the LCS computation
	 * @return The result as an array of 3 integers where result[0] is the x coordinate of the current
	 *         location in the diagonal with the most progress, result[1] is the y coordinate of the
	 *         current location in the diagonal with the most progress and result[2] is the amount of
