...
public class Group {
	...
	public Group(DocParseData data) {
		this.data = data;
	}

	/**
	 * Adds edges to the locus feature for those leaf features within the local space. The locus is, by
	 * definition, a node: termnial or comment.
	 */
	public void addLocalEdges(Feature locus) {
		...
		if (locus.getEdgeSet().isEmpty()) {
			...
			locus.addEdge(rootCtx, EdgeType.ADJACENT);
		}
	}
	...
	private void addEnclosing(ParserRuleContext ctx) {
		...
		while (ctx != null && idx < ENCLOSURES) {
			if (ctx.getParent() != null) { // no root feature
				...
				locus.addEdge(feature, EdgeType.KIN);
				addChildren(ctx);
			}
			ctx = ctx.getParent();
			idx++;
	...
	private void addAdjacent(Token token, int line) {
		...
		for (int i = mid - 1, cnt = 0; i >= 0 && cnt < ADJACENTS; i--) {
			...
			if (n != null) {
				...
				if (feature != null) {
					locus.addEdge(feature, EdgeType.ADJACENT);
					cnt++;
				}
			}
		}
				...
				for (Token token : tokens) {
					...
					if (Form.alignSame(data, start.getType(), token.getType())
						...
						if (node != null) {
							...
							locus.addEdge(feature, EdgeType.ALIGNED);
							break;
						}
					}
				}
		...
		for (int idx = beg; idx <= end; idx++) {
			...
			for (Token tok : tokens) {
				...
				if (node != null && type == tok.getType()) {
					Feature feature = data.terminalIndex.get(node);
					locus.addEdge(feature, EdgeType.SAME_TYPE);
				}
			}
		}
	}
