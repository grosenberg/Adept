...
public class Format {
	...
	public Format(ParseRecord data, Token token) {
		this.data = data;
		Info info = data.getDocument().getInfo(token.getLine());
	...
	private void characterize(Token token, Info info) {
		...
		int line = token.getLine(); // make zero indexed
	...
	private int leftWSVisualWidth(int idx) {
		List<Token> tokens = data.getHiddenTokensToLeft(idx);
		return wsVisualWidth(tokens, true);
	}

	private int rightWSVisualWidth(int idx) {
		List<Token> tokens = data.getHiddenTokensToRight(idx);
